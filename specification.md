...
4. **Interaction-net compiler** — Lafont's interaction combinators (γ constructor, δ duplicator, ε eraser) as a confluent reducer, plus a compiler from a fragment of Latte into nets, invokable on real source via `latte net "<expr>"` and narrated by `latte icomb`. The net's primitive agents are naturals under `+`, `*`, `<`, and `if`; on top of those the lowering accepts a richer source fragment by expanding it into the primitives at compile time — `let`, `+(…)`, `==`, **let-bound user functions** (inlined), and `loop … again(…)` as **bounded recursion** (unrolled to a fuel budget). The `if` is **lazy**: the closed condition is evaluated and only the taken branch is built into the net, so the untaken branch is never reduced. **Unbounded general recursion now runs on the net via HVM-style `Ref` nodes** — top-level definitions unrolled lazily by the reducer and *collected* against an eraser at the base case, so recursion terminates. A self-recursive `let f = fn [n] -> … (f …) … in (f K)` is detected and reduced as a genuine net-level fixpoint: factorial, sum, Fibonacci, and triangular numbers (100 levels deep) all agree with the interpreter. The reducer also uses an **active-pair worklist** instead of rescanning every step, cutting recursion-heavy reductions from seconds to tens of milliseconds (`tri 150` 4847 ms → 58 ms, `fac 8` 18760 ms → 17 ms) while preserving confluence. A fully dynamic lazy `if` (non-constant conditions, via IC boxes) and multi-argument net recursion remain future work (§11). Documented in `docs/interaction-nets.md`.
...
- **Orpheus GUI** — served by Hymn: a System console, a WYSIWYG Facet page editor with Unicode support, charts, the planner, an Oberon-style live module-compiler page, and the **system manuals served in-GUI at `/docs`**, which are now **editable in place** — an Edit/Save toolbar (Ctrl+S) writes the Markdown back through a path-validated `POST /api/doc`, in the Oberon spirit of every text being a live, editable document. The server also **hosts the complete Ligurian reference grammar** (Solar Speech, Vulgar Ligurian, and Heart Speech, with full paradigms, the hymns, and the vocabulary) at `/grammar`, rendered from Markdown and linked from the System page.
...
`std`; `num` (signed fixed-point); `tensor` (n-dimensional); `vec`; `ml` (linear regression by gradient descent, plus a **perceptron**, **k-means clustering**, and **k-NN** classifier — `latte ml [linear|perceptron|kmeans|knn]`); `nn` (**neural networks** — composable layers as data: `%dense`/`%relu`/`%tanh`/`%res` folded by `net_fwd`, residual blocks for ResNet-style nets, and a one-hidden-layer MLP trained by full **backpropagation**, `latte nn`); `fin` (**practical financial ML** in the spirit of Lopez de Prado — momentum + realized-volatility features, walk-forward validation that never shuffles, and a logistic-regression classifier; after finding price-only models near-chance on gold, the demo targets the market where momentum and volatility clustering are strongest, **Bitcoin**, and predicts the next-day **volatility regime** for a genuine out-of-sample edge over baseline; `latte fin`); `gfx` (a **graphics library** — scenes are lists of tagged shapes `%line`/`%rect`/`%circle`/`%poly`/`%text` over packed-RGB colours, rendered to SVG by the host; `latte gfx`); `gpu` (a **data-parallel GPU compute library** — buffers and kernels `map`/`zipWith`/`reduce`/`saxpy`/`dot`/`matmul`/`shade` as data, targeting an NVIDIA GeForce RTX 4070 Ti SUPER with a multi-core-CPU reference backend in this build; the backend is **auto-detected** — a present GPU accelerates ML/GUI, an absent one falls back to CPU; integrates with `nn`/`ml` via matmul and with `gfx` via a Mandelbrot shader; `latte gpu`); `sentiment` (**Loughran-McDonald news sentiment** — counts finance-specific positive/negative words and computes polarity = (pos−neg)/(pos+neg) in Latte fixed-point; `latte sentiment "<text>"`, an exogenous feature for information-driven markets); a **trading advisor** (`latte trade`) that calls the best model and recommends whether and how much to trade via momentum direction + **fractional Kelly** and **volatility-targeted** sizing, standing aside when the edge is not positive (a research demo, not advice); `plot` (SVG) with a host **multi-series line renderer** for loss/equity curves; `plan` (a Towards-a-New-Socialism-style economic planner); `chess` (a full engine) and `chessml` (a chess evaluator whose piece values are *learned by gradient descent in Latte*, now driving an **alpha-beta minimax search with a positional evaluation** rather than a one-ply greedy choice). A board-game tool runs machine players (their own compiled Latte cores) and human-vs-machine play.
...
### 10.7 Tools / commands
`eval`, `cli`, `repl`, `gui`, `serve`, `node`, `agent`, `selftest`, `bench`, `sca`/`evolve`, `mold`/`typecheck`, `mocha`, `plan`, `team`, `tensor`, `ml`, `nn`, `fin`, `chart`, `icomb`, `jit`, `game`, `rustc` (Anvil), `net`, and `cache`.

### 10.8 What remains (frontier)
A fully *dynamic* lazy `if` for non-constant conditions via interaction-net boxes (the net's `if` is already lazy for the closed conditions it evaluates, building only the taken branch) and **multi-argument** net-level recursion (single-argument natural recursion now runs as a genuine fixpoint via `Ref` unrolling — §10.2); a native desktop tiling UI (the browser GUI stands in for the Orpheus viewer/track/frame model); and bit-for-bit self-hosting of the whole compiler in Latte. *(Unbounded recursion on the net, and the intricate stress- and cluster-conditioned sound changes, once listed here are now implemented — see §10.2 and §10.6.)*
...
- **Interaction combinators remain an optional accelerator, not the canonical core**, precisely because they cannot express some lambda terms (affine variables forbid `λx.(x x)`); the built net compiler covers an arithmetic/control fragment now extended with genuine net-level recursion (single-argument fixpoints via `Ref` unrolling), and equivalence to Loom is re-established by differential testing per compilation.
